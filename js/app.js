...
function loadState(){
  const base = {
    ...
    // New football/personal development state slices (single source of truth)
    training:[],
    matches:[],
    goals:[],
    diet:{},
    tactics:[],
    achievements:[],
    profile:{ name:"", avatar:"", position:"", team:"", jerseyNumber:"", bio:"", preferredFoot:"" }
  };
  try {
...
function init(){
  applyTheme(); setupPWA(); initFirebase(); bindNav(); bindTopbar(); const hv=(location.hash||"").replace("#",""); if(["home","today","week","month","brain","ideas","habits","tools","pomodoro","training","goals","performance","diet","tactics","achievements","profile"].includes(hv)) activeView=hv; render(); scheduleOpenNotifications(); setInterval(updateNowLine, 30000); setInterval(updateClock, 1000); updateClock();
}
function bindNav(){
  $$(".nav-item").forEach(btn=>btn.addEventListener("click",()=>{ setView(btn.dataset.view); $("#sidebar").classList.remove("open"); }));
...
function setView(v){
  ...
  const labelEl=$(`[data-view="${v}"]`);
  $("#viewLabel").textContent=labelEl?labelEl.textContent.trim():"";
...
function render(){ 
  ...
  updateClock(); 
  const fn={home:renderHome,today:renderToday,week:renderWeek,month:renderMonth,brain:renderBrain,ideas:renderIdeas,habits:renderHabits,tools:renderTools,pomodoro:renderPomodoro,training:renderTraining,goals:renderGoals,performance:renderPerformance,diet:renderDiet,tactics:renderTactics,achievements:renderAchievements,profile:renderProfile}[activeView]; 
  fn(); 
  updateNowLine(); 
}
...
function bindProfile(){ 
  ...
  $("#addTpl").onclick=()=>{state.template.push({time:"12:00",type:"custom",text:""});save();renderProfile();}; 
...
function bindCloudUI(){
  ...
  $("#cloudLogout").onclick=async()=>{ if(fb.auth) await fb.auth.signOut(); alert("از Firebase خارج شدی. اطلاعات محلی روی همین دستگاه باقی می‌ماند."); renderProfile(); };
...
/* ============================================================
   NEW PAGE RENDERERS — Phase 2 Architecture
   ============================================================ */

/* ── Home Dashboard ── */
function renderHome(){
  const todayKey = iso(new Date());
  const todayData = state.days[todayKey] || defaultDay(new Date());
  ensureDayShape(todayData);
  const done = todayData.events.filter(e=>e.done).length;
  const total = todayData.events.length;
  const pct = total ? Math.round((done/total)*100) : 0;
  const water = todayData.water || 0;
  const mood = todayData.mood || "";
  const moodEmoji = mood ? ["☹","﹙","─","﹚","☺"][Number(mood)-1] : "—";
  const habitPct = habitPercent(new Date());
  const nextEvents = todayData.events.filter(e=>!e.done).sort((a,b)=>timeToMin(a.time)-timeToMin(b.time)).slice(0,2);

  // Training & matches today
  const todayTrainings = (state.training||[]).filter(t=>t.date===todayKey);
  const todayMatches = (state.matches||[]).filter(m=>m.date===todayKey);

  // Active goals count
  const activeGoals = (state.goals||[]).filter(g=>!g.completed).length;

  // Recent achievements
  const recentAchievements = (state.achievements||[]).slice(-3);
  ...
  $("#home").innerHTML = `
    <div class="home-dashboard">
        ...
        <div class="panel">
          <div class="panel-body">
            <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
              <div style="width:90px;height:90px;border-radius:50%;background:conic-gradient(var(--primary) ${pct}%, rgba(255,255,255,.5) 0);display:grid;place-items:center;box-shadow:var(--soft-shadow)">
                <div style="width:72px;height:72px;border-radius:50%;background:var(--surface-strong);display:grid;place-items:center;font-size:18px;font-weight:800">${pct}%</div>
              </div>
              <div>
                <h3 style="margin:0 0 4px;font-size:18px">پیشرفت امروز</h3>
                <p class="subtle" style="margin:0">${done} از ${total} کار انجام شده</p>
                ${nextEvents.length?`<p class="subtle" style="margin:4px 0 0;font-size:12px"><b>بعدی:</b> ${escapeHtml(nextEvents[0].text)} (${nextEvents[0].time})</p>`:""}
              </div>
            </div>
          </div>
        </div>
        <div class="panel">
          <div class="panel-body">
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;text-align:center">
              <div><div style="font-size:28px;margin-bottom:4px">💧</div><div style="font-size:16px;font-weight:700">${water}</div><div class="mini">لیوان آب</div></div>
              <div><div style="font-size:28px;margin-bottom:4px">${moodEmoji}</div><div style="font-size:16px;font-weight:700">${mood?"حال "+mood:"—"}</div><div class="mini">خلق امروز</div></div>
              <div><div style="font-size:28px;margin-bottom:4px">☷</div><div style="font-size:16px;font-weight:700">${Math.round(habitPct)}%</div><div class="mini">عادت‌ها</div></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Football Section -->
      <div>
        <h3 class="home-section-title">فوتبال</h3>
        <div class="home-grid four">
          <div class="home-card" data-nav="training">
            <div class="home-card-icon">⚽</div>
            <h3>تمرین و مسابقات</h3>
            <p>${todayTrainings.length+todayMatches.length?todayTrainings.length+" تمرین · "+todayMatches.length+" مسابقه امروز":"امروز برنامه‌ای نیست"}</p>
          </div>
          <div class="home-card" data-nav="goals">
            <div class="home-card-icon">🎯</div>
            <h3>اهداف و ماموریت‌ها</h3>
            <p>${activeGoals?activeGoals+" هدف فعال":"هدفی ثبت نشده"}</p>
          </div>
          <div class="home-card" data-nav="performance">
            <div class="home-card-icon">📊</div>
            <h3>عملکرد و آمار</h3>
            <p>تحلیل پیشرفت فوتبالی</p>
          </div>
          <div class="home-card" data-nav="achievements">
            <div class="home-card-icon">🏆</div>
            <h3>دستاوردها</h3>
            <p>${state.achievements?.length||0} دستاورد ثبت شده</p>
          </div>
        </div>
      </div>

      <!-- Mind & Planning Section -->
      <div>
        <h3 class="home-section-title">ذهن و برنامه‌ریزی</h3>
        <div class="home-grid four">
          <div class="home-card" data-nav="ideas">
            <div class="home-card-icon">✧</div>
            <h3>ایده‌ها</h3>
            <p>${(state.ideas?.work?.length||0)+(state.ideas?.personal?.length||0)} ایده ذخیره شده</p>
          </div>
          <div class="home-card" data-nav="brain">
            <div class="home-card-icon">◇</div>
            <h3>بارش فکری</h3>
            <p>${state.brain?.length||0} مورد</p>
          </div>
          <div class="home-card" data-nav="tools">
            <div class="home-card-icon">⌁</div>
            <h3>ابزارها</h3>
            <p>${state.tools?.length||0} ابزار</p>
          </div>
          <div class="home-card" data-nav="pomodoro">
            <div class="home-card-icon">◷</div>
            <h3>پومودورو</h3>
            <p>تمرکز و بهره‌وری</p>
          </div>
        </div>
      </div>
      ...
      <!-- Body & Habits Section -->
      <div>
        <h3 class="home-section-title">بدن و عادت‌ها</h3>
        <div class="home-grid two">
          <div class="home-card" data-nav="diet">
            <div class="home-card-icon">🥗</div>
            <h3>تغذیه و سلامت</h3>
            <p>خواب، انرژی، تغذیه و سلامت</p>
          </div>
          <div class="home-card" data-nav="habits">
            <div class="home-card-icon">☷</div>
            <h3>عادت‌ها</h3>
            <p>${state.habits?.length||0} عادت فعال</p>
          </div>
        </div>
      </div>
      ...
      <!-- Planning Section -->
      <div>
        <h3 class="home-section-title">برنامه‌ریزی</h3>
        <div class="home-grid three">
          <div class="home-card" data-nav="today">
            <div class="home-card-icon">◈</div>
            <h3>امروز</h3>
            <p>برنامه روزانه</p>
          </div>
          <div class="home-card" data-nav="week">
            <div class="home-card-icon">▤</div>
            <h3>هفته</h3>
            <p>نمای هفتگی</p>
          </div>
          <div class="home-card" data-nav="month">
            <div class="home-card-icon">▦</div>
            <h3>ماه</h3>
            <p>نمای ماهانه</p>
          </div>
      ...
      <!-- Profile -->
      <div>
        <h3 class="home-section-title">حساب کاربری</h3>
        <div class="home-grid two">
          <div class="home-card" data-nav="tactics">
            <div class="home-card-icon">📝</div>
            <h3>یادداشت و تاکتیک</h3>
            <p>${state.tactics?.length||0} یادداشت فوتبالی</p>
          </div>
          <div class="home-card" data-nav="profile">
            <div class="home-card-icon">⚙</div>
            <h3>پروفایل و تنظیمات</h3>
            <p>${state.profile?.name||"تنظیمات و اطلاعات کاربری"}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  $$(".home-card[data-nav]").forEach(card=>{
    card.addEventListener("click",()=>setView(card.dataset.nav));
  });
}

/* ── Training & Matches Calendar ── */
function renderTraining(){
  const list = (state.training||[]).sort((a,b)=>a.date.localeCompare(b.date)||timeToMin(a.time)-timeToMin(b.time));
  const matchList = (state.matches||[]).sort((a,b)=>a.date.localeCompare(b.date)||timeToMin(a.time)-timeToMin(b.time));

  const trainingHtml = list.length ? list.map((t,i)=>`
    <div class="list-item" style="grid-template-columns:1fr auto auto">
      <div>
        <b>${escapeHtml(t.title||"تمرین")}</b>
        <div class="mini">${t.date} · ${t.time} · ${escapeHtml(t.type||"")} · ${escapeHtml(t.location||"")}</div>
        ${t.notes?`<div class="mini" style="margin-top:4px">${escapeHtml(t.notes)}</div>`:""}
      </div>
      <span class="chip ${t.completed?'primary-chip':''}">${t.completed?'✓ انجام شد':'در انتظار'}</span>
      <button class="small-btn" data-training-del="${i}">×</button>
    </div>
  `).join("") : `<p class="subtle">هنوز تمرینی ثبت نشده. اولین تمرین را اضافه کن.</p>`;

  const matchHtml = matchList.length ? matchList.map((m,i)=>`
    <div class="list-item" style="grid-template-columns:1fr auto auto">
      <div>
        <b>مسابقه vs ${escapeHtml(m.opponent||"؟")}</b>
        <div class="mini">${m.date} · ${m.time} · ${escapeHtml(m.location||"")} · پست: ${escapeHtml(m.position||"—")}</div>
        ${m.minutes!==undefined?`<div class="mini" style="margin-top:4px">${m.minutes} دقیقه · ${m.goals||0} گل · ${m.assists||0} پاس گل</div>`:""}
        ${m.notes?`<div class="mini">${escapeHtml(m.notes)}</div>`:""}
      </div>
      <span class="chip ${m.completed?'primary-chip':''}">${m.completed?'✓ برگزار شد':'در پیش'}</span>
      <button class="small-btn" data-match-del="${i}">×</button>
    </div>
  `).join("") : `<p class="subtle">هنوز مسابقه‌ای ثبت نشده.</p>`;

  $("#training").innerHTML =
    card("تمرین و مسابقات",
      `<div class="form-grid" style="margin-bottom:14px">
        <input id="trTitle" placeholder="عنوان تمرین / مسابقه">
        <select id="trType"><option value="training">تمرین</option><option value="match">مسابقه</option><option value="recovery">بازیابی</option><option value="rest">استراحت</option><option value="gym">بدنسازی</option><option value="other">سایر</option></select>
        <input id="trDate" type="date" value="${iso(selected)}">
        <input id="trTime" type="time" value="10:00">
        <input id="trLocation" placeholder="مکان">
        <input id="trOpponent" placeholder="حریف (فقط مسابقه)">
        <input id="trPosition" placeholder="پست بازی">
        <input id="trMinutes" type="number" placeholder="دقایق بازی">
        <input id="trGoals" type="number" placeholder="گل">
        <input id="trAssists" type="number" placeholder="پاس گل">
        <textarea id="trNotes" class="full" placeholder="یادداشت..."></textarea>
        <button id="addTraining" class="premium-btn full">+ ثبت</button>
      </div>
      <h3 style="margin:18px 0 10px;font-size:15px">📋 تمرین‌ها</h3>
      ${trainingHtml}
      <h3 style="margin:18px 0 10px;font-size:15px">⚽ مسابقات</h3>
      ${matchHtml}`
    );
  ...
  $("#addTraining").onclick = () => {
    ...
    const time = $("#trTime").value;
    const title = $("#trTitle").value.trim() || (type==="match"?"مسابقه":"تمرین");
    const location = $("#trLocation").value.trim();
    const notes = $("#trNotes").value.trim();
    const entry = { id:uid(), type, date, time, title, location, notes, completed:false, createdAt:Date.now() };
    if(type==="match"){
      entry.opponent = $("#trOpponent").value.trim();
      entry.position = $("#trPosition").value.trim();
      entry.minutes = Number($("#trMinutes").value)||0;
      entry.goals = Number($("#trGoals").value)||0;
      entry.assists = Number($("#trAssists").value)||0;
      state.matches = state.matches||[];
      state.matches.push(entry);
    } else {
      state.training = state.training||[];
      state.training.push(entry);
    }
    save();
    renderTraining();
  };

  $$("[data-training-del]").forEach(b=>b.onclick=()=>{
    state.training.splice(Number(b.dataset.trainingDel),1);
    save(); renderTraining();
  });
  $$("[data-match-del]").forEach(b=>b.onclick=()=>{
    state.matches.splice(Number(b.dataset.matchDel),1);
    save(); renderTraining();
  });
}
...
function renderGoals(){
  ...
  const goalCard = (g,gi)=>{
    ...
    const missionPct = missions.length ? Math.round((missions.filter(m=>m.completed).length/missions.length)*100) : 0;
    return `<div class="panel" style="margin-bottom:14px">
      <div class="panel-head">
        <h2>${escapeHtml(g.title)} <span class="mini">${missionPct}%</span></h2>
        <div class="panel-actions">
          <button class="small-btn" data-goal-toggle="${gi}">${g.completed?'↩ بازگردانی':'✓ تکمیل'}</button>
          <button class="small-btn" data-goal-del="${gi}">×</button>
        </div>
      </div>
      <div class="panel-body">
        <p class="subtle" style="margin-bottom:10px">${escapeHtml(g.description||"")}</p>
        <div style="height:6px;background:rgba(37,40,32,.09);border-radius:99px;overflow:hidden;margin-bottom:12px">
          <div style="height:100%;background:var(--primary);width:${missionPct}%;transition:width .3s ease"></div>
        </div>
        ${missions.map((m,mi)=>`<div class="list-item" style="grid-template-columns:1fr auto auto;padding:10px 12px">
          <div style="display:flex;align-items:center;gap:8px">
            <input type="checkbox" data-goal="${gi}" data-mission="${mi}" ${m.completed?'checked':''}>
            <span style="${m.completed?'text-decoration:line-through;opacity:.5':''}">${escapeHtml(m.title)}</span>
          </div>
          <span class="mini">${(m.actions||[]).filter(a=>a.done).length}/${(m.actions||[]).length} عمل</span>
          <button class="small-btn" data-mission-del="${gi}-${mi}">×</button>
        </div>`).join("")}
        <div class="input-row" style="margin-top:10px">
          <input placeholder="ماموریت جدید..." data-goal-add-mission="${gi}">
          <button class="small-btn" data-goal-add-mission-btn="${gi}">+</button>
        </div>
      </div>
    </div>`;
  };
  ...
  $("#goals").innerHTML =
    card("اهداف و ماموریت‌ها",
      `<div class="input-row" style="margin-bottom:16px">
        <input id="newGoalTitle" placeholder="عنوان هدف (مثلاً: تقویت پای ضعیف)">
        <button id="addGoal" class="premium-btn">+ هدف</button>
      </div>
      <textarea id="newGoalDesc" placeholder="توضیح هدف..." style="min-height:60px;margin-bottom:16px"></textarea>
      <h3 style="margin:0 0 10px;font-size:14px">🎯 اهداف فعال</h3>
      ${active.length?active.map((g,i)=>goalCard(g,goals.indexOf(g))).join(""):"<p class="subtle">هدفی ثبت نشده.</p>"}
      ${done.length?`<h3 style="margin:18px 0 10px;font-size:14px">✓ تکمیل شده</h3>${done.map((g,i)=>goalCard(g,goals.indexOf(g))).join("")}`:""}`
    );
  ...
  $("#addGoal").onclick = () => {
    ...
    state.goals.push({ id:uid(), title, description:$("#newGoalDesc").value.trim(), category:"football", createdAt:iso(new Date()), completed:false, missions:[] });
    $("#newGoalTitle").value=""; $("#newGoalDesc").value="";
    save(); renderGoals();
  };

  $$("[data-goal-del]").forEach(b=>b.onclick=()=>{ state.goals.splice(Number(b.dataset.goalDel),1); save(); renderGoals(); });
  $$("[data-goal-toggle]").forEach(b=>b.onclick=()=>{ const g=state.goals[Number(b.dataset.goalToggle)]; g.completed=!g.completed; save(); renderGoals(); });
  $$("[data-mission-del]").forEach(b=>b.onclick=()=>{ const [gi,mi]=b.dataset.missionDel.split("-"); state.goals[gi].missions.splice(mi,1); save(); renderGoals(); });
  $$("[data-goal-add-mission-btn]").forEach(b=>b.onclick=()=>{
    const gi=b.dataset.goalAddMissionBtn;
    const inp=$(`[data-goal-add-mission="${gi}"]`);
    const title=inp.value.trim(); if(!title)return;
    state.goals[gi].missions=state.goals[gi].missions||[];
    state.goals[gi].missions.push({ id:uid(), title, completed:false, actions:[] });
    save(); renderGoals();
  });
  $$('input[data-goal][data-mission]').forEach(cb=>cb.onchange=()=>{
    const g=state.goals[cb.dataset.goal]; const m=g.missions[cb.dataset.mission];
    m.completed=cb.checked; save(); renderGoals();
  });
}

/* ── Performance & Football Stats ── */
function renderPerformance(){
  const goals = state.goals||[];
  const totalGoals = goals.length;
  const completedGoals = goals.filter(g=>g.completed).length;
  const totalMissions = goals.reduce((s,g)=>s+(g.missions||[]).length,0);
  const completedMissions = goals.reduce((s,g)=>s+(g.missions||[]).filter(m=>m.completed).length,0);

  const matches = state.matches||[];
  const totalMatches = matches.length;
  const totalGoalsScored = matches.reduce((s,m)=>s+(m.goals||0),0);
  const totalAssists = matches.reduce((s,m)=>s+(m.assists||0),0);
  const totalMinutes = matches.reduce((s,m)=>s+(m.minutes||0),0);

  // Weekly habit consistency (last 7 days)
  const last7 = Array.from({length:7},(_,i)=>addDays(new Date(),-6+i));
  const habitConsistency = state.habits ? Math.round(last7.reduce((sum,d)=>sum+habitPercent(d),0)/7) : 0;

  $("#performance").innerHTML = `
    <div class="grid two">
      <div class="panel">
        <div class="panel-body">
          <h3 style="margin:0 0 12px;font-size:16px">📊 آمار کلی</h3>
          <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px">
            <div style="text-align:center;padding:14px;background:rgba(255,255,255,.5);border-radius:18px">
              <div style="font-size:28px;font-weight:800;color:var(--primary)">${completedGoals}/${totalGoals}</div>
              <div class="mini">اهداف تکمیل شده</div>
            </div>
            <div style="text-align:center;padding:14px;background:rgba(255,255,255,.5);border-radius:18px">
              <div style="font-size:28px;font-weight:800;color:var(--primary)">${completedMissions}/${totalMissions}</div>
              <div class="mini">ماموریت‌های انجام شده</div>
            </div>
            <div style="text-align:center;padding:14px;background:rgba(255,255,255,.5);border-radius:18px">
              <div style="font-size:28px;font-weight:800;color:var(--primary)">${totalMatches}</div>
              <div class="mini">مسابقات</div>
            </div>
            <div style="text-align:center;padding:14px;background:rgba(255,255,255,.5);border-radius:18px">
              <div style="font-size:28px;font-weight:800;color:var(--primary)">${totalGoalsScored}</div>
              <div class="mini">گل زده</div>
            </div>
            <div style="text-align:center;padding:14px;background:rgba(255,255,255,.5);border-radius:18px">
              <div style="font-size:28px;font-weight:800;color:var(--primary)">${totalAssists}</div>
              <div class="mini">پاس گل</div>
            </div>
            <div style="text-align:center;padding:14px;background:rgba(255,255,255,.5);border-radius:18px">
              <div style="font-size:28px;font-weight:800;color:var(--primary)">${totalMinutes}</div>
              <div class="mini">دقیقه بازی</div>
            </div>
          </div>
        </div>
      </div>
      <div class="panel">
        <div class="panel-body">
          <h3 style="margin:0 0 12px;font-size:16px">📈 پیشرفت هفتگی</h3>
          <div style="text-align:center;padding:20px">
            <div style="font-size:42px;font-weight:800;color:var(--primary)">${habitConsistency}%</div>
            <div class="mini">میانگین پایداری عادت‌ها (۷ روز)</div>
          </div>
          <canvas id="perfHabitChart" class="chart"></canvas>
        </div>
      </div>
    </div>
    <div class="panel" style="margin-top:18px">
      <div class="panel-body">
        <h3 style="margin:0 0 12px;font-size:16px">⚽ روند مسابقات</h3>
        <canvas id="perfMatchChart" class="chart"></canvas>
      </div>
    </div>
  `;

  // Draw habit consistency chart
  drawBars("perfHabitChart", last7.map(d=>Math.round(habitPercent(d))), last7.map(d=>WEEKDAYS[(d.getDay()+1)%7]), "پایداری عادت‌ها");

  // Draw match performance chart (goals + assists over matches)
  const matchData = matches.slice(-10);
  if(matchData.length && $("#perfMatchChart")){
    const c=$("#perfMatchChart");
    const ctx=c.getContext("2d"), w=c.width=c.clientWidth*devicePixelRatio, h=c.height=c.clientHeight*devicePixelRatio;
    ctx.scale(devicePixelRatio,devicePixelRatio);
    const cw=c.clientWidth, ch=c.clientHeight;
    ctx.clearRect(0,0,cw,ch);
    const padX=30, padY=25, chartW=cw-padX*2, chartH=ch-padY*2;
    const maxVal=Math.max(1,...matchData.map(m=>(m.goals||0)+(m.assists||0)));
    const stepX=chartW/Math.max(1,matchData.length-1);

    // Grid
    ctx.strokeStyle="rgba(37,40,32,.08)"; ctx.lineWidth=1;
    for(let i=0;i<=4;i++){ const y=padY+chartH*(i/4); ctx.beginPath(); ctx.moveTo(padX,y); ctx.lineTo(padX+chartW,y); ctx.stroke(); }

    // Bars (goals + assists stacked)
    const barW=Math.min(stepX*.5,20);
    matchData.forEach((m,i)=>{
      const x=padX+(matchData.length===1?chartW/2:i*stepX);
      const gH=((m.goals||0)/maxVal)*chartH;
      const aH=((m.assists||0)/maxVal)*chartH;
      ctx.fillStyle=getComputedStyle(document.documentElement).getPropertyValue("--primary")||"#59684f";
      ctx.fillRect(x-barW/2, padY+chartH-gH, barW, gH);
      ctx.fillStyle=getComputedStyle(document.documentElement).getPropertyValue("--accent")||"#7b0d45";
      ctx.fillRect(x-barW/2, padY+chartH-gH-aH, barW, aH);
    });
    ...
    matchData.forEach((m,i)=>{
      const x=padX+(matchData.length===1?chartW/2:i*stepX);
      ctx.fillText(m.opponent||"#"+(i+1), x, padY+chartH+16);
    });
  }
}
...
function renderDiet(){
  ...
  const d = state.diet[key] || { meals:[], sleep:"", energy:"", recovery:"", notes:"", preTraining:"", postTraining:"" };

  $("#diet").innerHTML = `
    <div class="grid two">
      <div class="panel">
        <div class="panel-head"><h2>🍽 وعده‌های غذایی</h2></div>
        <div class="panel-body">
          <div class="input-row">
            <input id="mealName" placeholder="نام وعده (صبحانه، ناهار، شام...)">
            <button id="addMeal" class="small-btn">+</button>
          </div>
          <textarea id="mealNote" placeholder="جزئیات تغذیه‌ای..." style="min-height:60px;margin-bottom:12px"></textarea>
          ${(d.meals||[]).map((m,i)=>`<div class="list-item" style="grid-template-columns:1fr auto">
            <div><b>${escapeHtml(m.name)}</b><div class="mini">${escapeHtml(m.note||"")}</div></div>
            <button class="small-btn" data-meal-del="${i}">×</button>
          </div>`).join("")}
          <div style="margin-top:14px;padding:12px;background:rgba(255,255,255,.45);border-radius:16px">
            <b class="mini" style="display:block;margin-bottom:6px">🥤 آب امروز (از صفحه امروز)</b>
            <div style="font-size:22px;font-weight:800">${(state.days[key]?.water||0)} لیوان</div>
          </div>
        </div>
      </div>
      <div class="panel">
        <div class="panel-head"><h2>💤 سلامت و بازیابی</h2></div>
        <div class="panel-body">
          <div class="form-grid">
            <label>ساعت خواب<input id="sleepHours" type="number" step="0.5" value="${d.sleep||""}" placeholder="مثلاً 7.5"></label>
            <label>انرژی (۱-۵)<input id="dietEnergy" type="number" min="1" max="5" value="${d.energy||""}"></label>
            <label>بازیابی (۱-۵)<input id="dietRecovery" type="number" min="1" max="5" value="${d.recovery||""}"></label>
            <label>خلق امروز (از صفحه امروز)
              <div style="padding:10px;background:rgba(255,255,255,.5);border-radius:14px;text-align:center;font-size:18px">
                ${state.days[key]?.mood?["☹","﹙","─","﹗","☺"][Number(state.days[key].mood)-1]+" (حال "+state.days[key].mood+")":"—"}
              </div>
            </label>
          </div>
          <textarea id="dietNotes" placeholder="یادداشت سلامت..." style="min-height:80px;margin-top:10px">${escapeHtml(d.notes||"")}</textarea>
          <div class="form-grid" style="margin-top:10px">
            <textarea id="preTraining" placeholder="تغذیه قبل از تمرین..." style="min-height:60px">${escapeHtml(d.preTraining||"")}</textarea>
            <textarea id="postTraining" placeholder="تغذیه بعد از تمرین..." style="min-height:60px">${escapeHtml(d.postTraining||"")}</textarea>
          </div>
        </div>
      </div>
    </div>
  ...
  $("#addMeal").onclick = () => {
    const name = $("#mealName").value.trim(); if(!name) return;
    state.diet[key] = state.diet[key] || { meals:[], sleep:"", energy:"", recovery:"", notes:"", preTraining:"", postTraining:"" };
    state.diet[key].meals.push({ name, note:$("#mealNote").value.trim() });
    $("#mealName").value=""; $("#mealNote").value="";
    save(); renderDiet();
  };
  $$("[data-meal-del]").forEach(b=>b.onclick=()=>{
    state.diet[key].meals.splice(Number(b.dataset.mealDel),1);
    save(); renderDiet();
  });

  const bindDietField = (id,field)=>{ const el=$(id); if(!el)return; el.onchange=()=>{ state.diet[key]=state.diet[key]||{meals:[],sleep:"",energy:"",recovery:"",notes:"",preTraining:"",postTraining:""}; state.diet[key][field]=el.value; save(); }; };
  bindDietField("#sleepHours","sleep");
  bindDietField("#dietEnergy","energy");
  bindDietField("#dietRecovery","recovery");
  bindDietField("#dietNotes","notes");
  bindDietField("#preTraining","preTraining");
  bindDietField("#postTraining","postTraining");
}

/* ── Notes & Tactics ── */
function renderTactics(){
  const list = state.tactics||[];
  const categories = ["تاکتیک","فرماسیون","یادداشت حریف","بازخورد مربی","پست","یادداشت تمرین","ضربات ایستگاهی","مشاهدات شخصی"];

  $("#tactics").innerHTML =
    card("یادداشت و تاکتیک فوتبالی",
      `<div class="form-grid" style="margin-bottom:14px">
        <input id="tacTitle" placeholder="عنوان">
        <select id="tacCategory">${categories.map(c=>`<option value="${c}">${c}</option>`).join("")}</select>
        <textarea id="tacContent" class="full" placeholder="محتوای یادداشت..." style="min-height:120px"></textarea>
        <input id="tacTags" class="full" placeholder="برچسب‌ها (با کاما جدا کنید)">
        <button id="addTactic" class="premium-btn full">+ ثبت یادداشت</button>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px">
        ${categories.map(c=>`<button class="chip ${c===($('#tacFilter')?.value||'')?'primary-chip':''}" data-tac-filter="${c}">${c}</button>`).join("")}
        <button class="chip ${!$('#tacFilter')?.value?'primary-chip':''}" data-tac-filter="">همه</button>
      </div>
      ${list.length?list.map((t,i)=>`<div class="panel" style="margin-bottom:10px">
        <div class="panel-head">
          <h2 style="font-size:15px">${escapeHtml(t.title)} <span class="chip" style="font-size:11px;padding:4px 8px">${escapeHtml(t.category)}</span></h2>
          <div class="panel-actions">
            <button class="small-btn" data-tac-del="${i}">×</button>
          </div>
        </div>
        <div class="panel-body">
          <div style="white-space:pre-wrap;font-size:13px;line-height:1.7;margin-bottom:8px">${escapeHtml(t.content)}</div>
          ${t.tags?`<div style="display:flex;flex-wrap:wrap;gap:4px">${t.tags.split(",").map(tag=>`<span class="chip" style="font-size:11px;padding:4px 8px">${escapeHtml(tag.trim())}</span>`).join("")}</div>`:""}
          <div class="mini" style="margin-top:8px">${t.createdAt||""}</div>
        </div>
      </div>`).join(""):"<p class="subtle">هنوز یادداشتی ثبت نشده.</p>"}`
    );

  $("#addTactic").onclick = () => {
    const title = $("#tacTitle").value.trim(); if(!title) return;
    state.tactics = state.tactics||[];
    state.tactics.unshift({ id:uid(), title, category:$("#tacCategory").value, content:$("#tacContent").value.trim(), tags:$("#tacTags").value.trim(), createdAt:iso(new Date()) });
    $("#tacTitle").value=""; $("#tacContent").value=""; $("#tacTags").value="";
    save(); renderTactics();
  };
  $$("[data-tac-del]").forEach(b=>b.onclick=()=>{ state.tactics.splice(Number(b.dataset.tacDel),1); save(); renderTactics(); });
}
...
function renderAchievements(){
  ...
  $("#achievements").innerHTML =
    card("دستاوردها",
      `<div class="form-grid" style="margin-bottom:16px">
        <input id="achTitle" placeholder="عنوان دستاورد">
        <input id="achCategory" placeholder="دسته (فوتبالی، شخصی، عادت...)">
        <textarea id="achDesc" class="full" placeholder="توضیح..." style="min-height:60px"></textarea>
        <input id="achDate" type="date" class="full" value="${iso(selected)}">
        <button id="addAchievement" class="premium-btn full">+ ثبت دستاورد</button>
      </div>
      <div class="home-grid three">
        ${list.length?list.slice().reverse().map((a,i)=>`<div class="panel" style="text-align:center;padding:20px">
          <div style="font-size:40px;margin-bottom:10px">${a.icon||"🏆"}</div>
          <h3 style="margin:0 0 6px;font-size:15px">${escapeHtml(a.title)}</h3>
          <p class="subtle" style="margin:0 0 8px;font-size:12px">${escapeHtml(a.description||"")}</p>
          <span class="chip" style="font-size:11px">${escapeHtml(a.category||"")} · ${a.date||""}</span>
          <button class="small-btn" style="margin-top:10px" data-ach-del="${list.length-1-i}">حذف</button>
        </div>`).join(""):"<p class="subtle" style="grid-column:1/-1;text-align:center">هنوز دستاوردی ثبت نشده.</p>"}
      </div>`
    );
  ...
  $("#addAchievement").onclick = () => {
    ...
    state.achievements.push({ id:uid(), title, description:$("#achDesc").value.trim(), category:$("#achCategory").value.trim(), date:$("#achDate").value, icon:"🏆", createdAt:Date.now() });
    $("#achTitle").value=""; $("#achDesc").value=""; $("#achCategory").value="";
    save(); renderAchievements();
  };
  $$("[data-ach-del]").forEach(b=>b.onclick=()=>{ state.achievements.splice(Number(b.dataset.achDel),1); save(); renderAchievements(); });
}
