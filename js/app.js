...
function applyTheme(){
  ...
  if(s.bgImage) {
    document.documentElement.style.setProperty("--custom-bg", `linear-gradient(rgba(242,238,232,.80),rgba(242,238,232,.80)), url(${s.bgImage})`);
  } else {
...
function render(){ $("#datePicker").value=iso(selected); $("#currentDateLabel").textContent=persianLabel(selected); $("#gregorianDateLabel").textContent=new Intl.DateTimeFormat("en-GB",{dateStyle:"full"}).format(selected); updateClock(); const fn={today:renderToday,week:renderWeek,month:renderMonth,brain:renderBrain,ideas:renderIdeas,habits:renderHabits,tools:renderTools,pomodoro:renderPomodoro,settings:renderSettings}[activeView]; fn(); updateNowLine(); }
...
function renderToday(){
  ...
  $("#today").innerHTML = `<div class="today-stack"><div>${schedule}</div><div class="grid two today-lower"><div>${photos}${reflection}</div><div class="grid">${side}${notes}</div></div></div>`;
...
function voiceHtml(d){ return `<div class="section voice-widget"><div class="voice-controls"><button id="recBtn" class="tiny-control" title="شروع ضبط">◉</button><input id="voiceUrl" placeholder="لینک صوتی..."><button id="addVoice" class="tiny-control" title="ذخیره لینک">✓</button></div><div class="list compact-list">${(d.voiceLinks||[]).map((v,i)=>`<div class="list-item"><a href="${v.url}" target="_blank">${v.title||"voice note"}</a><button class="tiny-icon" data-voice-del="${i}">×</button></div>`).join("")}</div></div>`; }
function bindVoice(){ let recorder,chunks=[]; $("#recBtn").onclick=async()=>{ const btn=$("#recBtn"); if(recorder&&recorder.state==="recording"){ recorder.stop(); btn.textContent="شروع ضبط"; return; } const stream=await navigator.mediaDevices.getUserMedia({audio:true}); recorder=new MediaRecorder(stream); chunks=[]; recorder.ondataavailable=e=>chunks.push(e.data); recorder.onstop=()=>{ const blob=new Blob(chunks,{type:"audio/webm"}); const fr=new FileReader(); fr.onload=()=>{const d=dayState(selected); d.voiceLinks=d.voiceLinks||[]; d.voiceLinks.push({title:`یادداشت صوتی ${new Date().toLocaleTimeString("fa-IR")}`,url:fr.result}); save(); renderToday();}; fr.readAsDataURL(blob); stream.getTracks().forEach(t=>t.stop()); }; recorder.start(); btn.textContent="توقف ضبط"; }; $("#addVoice").onclick=()=>{const url=$("#voiceUrl").value.trim(); if(!url)return; const d=dayState(selected); d.voiceLinks=d.voiceLinks||[]; d.voiceLinks.push({title:url,url}); save(); renderToday();}; $$("[data-voice-del]").forEach(b=>b.onclick=()=>{const d=dayState(selected); d.voiceLinks.splice(Number(b.dataset.voiceDel),1); save(); renderToday();}); }
...
function renderWeek(){
  const start=addDays(selected,-((selected.getDay()+1)%7)); const days=Array.from({length:7},(_,i)=>addDays(start,i));
  const rows=days.map(d=>{const ds=dayState(d), done=ds.events.filter(e=>e.done).length, total=ds.events.length, habits=habitPercent(d), photos=(ds.photos||[]).length; return `<tr><td>${persianLabel(d)}</td><td>${done}/${total}</td><td>${Math.round(habits)}٪</td><td>${ds.mood||"—"}</td><td>${photos} عکس</td><td><button class="small-btn" data-goto="${iso(d)}">باز کردن</button></td></tr>`}).join("");
  ...
  $("#week").innerHTML=card("داشبورد هفتگی",`<table class="week-table"><thead><tr><th>روز</th><th>تیک‌ها</th><th>عادت‌ها</th><th>خلق</th><th>عکس</th><th></th></tr></thead><tbody>${rows}</tbody></table><canvas id="weekChart" class="chart"></canvas>`)+card("روند خلق و انرژی هفتگی",`<canvas id="moodWeekChart" class="chart"></canvas>`)+card("گالری عکس‌های هفته",`<div class="photo-polaroid-grid">${gallery||"<p class=\"subtle\">هنوز عکسی برای این هفته ثبت نشده.</p>"}</div>`);
  ...
  drawMoodChart("moodWeekChart", days);
}
...
function renderMonth(){
  ...
  $("#month").innerHTML=card(`نمای ماهانه ${PERSIAN_MONTHS[j.jm-1]} ${j.jy}`,`<div class="month-grid">${cells}</div>`)+card("روند خلق و انرژی ماهانه",`<canvas id="moodMonthChart" class="chart"></canvas>`)+card("گالری عکس‌های ماه",`<div class="photo-polaroid-grid">${monthPhotos||"<p class=\"subtle\">هنوز عکسی برای این ماه ثبت نشده.</p>"}</div>`); $$(".month-day").forEach(b=>b.onclick=()=>{selected=fromIso(b.dataset.date);setView("today");});
  const monthDays = Array.from({length:len},(_,i)=>toGregorian(j.jy,j.jm,i+1));
  drawMoodChart("moodMonthChart", monthDays);
}
...
function renderHabits(){ 
  ...
  drawBars("habitChart", state.habits.map(h=>habitMonthPercent(j.jy,j.m,h)), state.habits); 
}
...
function renderPomodoro(){ $("#pomodoro").innerHTML=card("تایمر پومودورو داخلی",`<div class="timer-face" id="timerFace">${formatSeconds(pomodoro.seconds)}</div><div class="actions" style="justify-content:center"><button id="startTimer" class="premium-btn">${pomodoro.running?"توقف":"شروع"}</button><button id="resetTimer" class="premium-btn muted">ریست</button><select id="timerMode"><option value="focus">۲۵ دقیقه تمرکز</option><option value="short">۵ دقیقه استراحت</option><option value="long">۱۵ دقیقه استراحت بلند</option></select></div>`); $("#startTimer").onclick=toggleTimer; $("#resetTimer").onclick=()=>{stopTimer(); pomodoro.seconds=pomodoro.mode==="focus"?1500:pomodoro.mode==="short"?300:900; renderPomodoro();}; $("#timerMode").value=pomodoro.mode; $("#timerMode").onchange=e=>{pomodoro.mode=e.target.value; stopTimer(); pomodoro.seconds=pomodoro.mode==="focus"?1500:pomodoro.mode==="short"?300:900; renderPomodoro();}; }
...
function drawBars(id, values, labels){ 
  ...
  values.forEach((v,i)=>{
    const bw=w/values.length*.55, x=(i+.25)*w/values.length, bh=(h-45*devicePixelRatio)*v/max; 
    ctx.fillStyle=getComputedStyle(document.documentElement).getPropertyValue("--primary"); 
    ctx.fillRect(x,h-25*devicePixelRatio-bh,bw,bh); 
    ctx.fillStyle="#555"; 
    ctx.fillText(String(v),x,h-30*devicePixelRatio-bh);
    if(labels && labels[i]){
      ctx.save();
      ctx.fillStyle="#777";
      ctx.font=`${10*devicePixelRatio}px sans-serif`;
      ctx.fillText(labels[i], x, h-5*devicePixelRatio);
      ctx.restore();
    }
  }); 
}

function drawMoodChart(id, days){
  const c=$("#"+id); if(!c)return;
  ...
  const maxMood = 5, maxEnergy = 5;
  const padX = 40*devicePixelRatio, padY = 30*devicePixelRatio;
  ...
  moods.forEach((m,i)=>{
    ...
    const y = padY + chartH - (m/maxMood)*chartH;
  ...
  ctx.fillRect(padX, 8*devicePixelRatio, 12*devicePixelRatio, 4*devicePixelRatio);
  ctx.fillStyle = "#555"; ctx.font = `${10*devicePixelRatio}px sans-serif`; ctx.textAlign = "left";
  ctx.fillText("خلق", padX + 16*devicePixelRatio, 13*devicePixelRatio);
  ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue("--primary") || "#59684f";
  ctx.globalAlpha = 0.5;
  ctx.fillRect(padX + 60*devicePixelRatio, 8*devicePixelRatio, 12*devicePixelRatio, 4*devicePixelRatio);
  ctx.globalAlpha = 1;
  ctx.fillStyle = "#555";
  ctx.fillText("انرژی", padX + 76*devicePixelRatio, 13*devicePixelRatio);
}
